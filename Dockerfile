...
RUN pip install \
    ...
    azul-scaler==$(cd /tmp/src && python -m setuptools_scm)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    pip install --find-links /tmp/ azul-scaler==$(cd /tmp/src && python -m setuptools_scm);fi
...
RUN --mount=type=secret,uid=$UID,gid=$GID,id=testSecret export $(cat /run/secrets/testSecret) && \
    python -m pytest --tb=short --asyncio-mode=auto /tmp/tests
